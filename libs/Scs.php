...
class Scs
{
    ...
    public function uploadImage($img_file, $img_from, $img_fromid, $meta_data = array())
    {
        ...
        $imgData = array(
            ...
            'imgExt' => $image_ext,
            'imgWidth' => $this->size[0] ? $this->size[0] : 0,
            'imgHeight' => $this->size[1] ? $this->size[1] : 0,
        ...
        $imageArrPath = $imageArrFold . $image_id . "." . $image_ext;
        
        if(!move_uploaded_file($this->file['tmp_name'], $imageArrPath))
        {
            $objImage->rollback();
