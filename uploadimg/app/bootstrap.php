...
$di->set('dispatcher', function () use ($di) {
    ...
    $eventsManager->attach("dispatch:beforeException", function ($event, $dispatcher, $exception) {
        Log::ErrorWrite('www', '', $_SERVER['REQUEST_URI'] . "\t" . $exception->getMessage(), 'debug.txt');
...
$di->set('modelsMetadata', function () {
    ...
    return new \Phalcon\Mvc\Model\MetaData\Files(array(
        "lifetime" => 86400,
        'metaDataDir' => realpath(__DIR__) . '/compiled/'
    ));
});
...
try
{
    register_shutdown_function(array('Error', 'catchFatalError'), 'www');
    ...
    $UseTime >= 2 && Log::ErrorWrite('image', '', $_SERVER['REQUEST_URI'] . "\t共耗时 $UseTime 秒\n", 'timer.txt');
} catch(Exception $e)
{
    ...
    Log::ErrorWrite('image', '', $_SERVER['REQUEST_URI'] . "\t" . "错误代码：" . $e->getCode() . "\r 错误信息：" . $e->getMessage() . '\r 文件：' . $e->getFile() . '\r 行号：' . $e->getLine() . '\r 路由信息：' . $e->getTraceAsString(), 'debug.txt');
